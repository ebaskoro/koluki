/**
 * References
 *
 */

/// <reference path="../../typings/angularjs/angular.d.ts" />
/// <reference path="../../typings/angularjs/angular-route.d.ts" />
/// <reference path="../../typings/angularjs/angular-animate.d.ts" />
/// <reference path="../../typings/angularjs/angular-resource.d.ts" />
/// <reference path="../../typings/angular-ui-bootstrap/angular-ui-bootstrap.d.ts" />
/// <reference path="../../typings/angularjs-toaster/angularjs-toaster.d.ts" />

/// <reference path="RouteConfig.ts" />
/// <reference path="models/Slide.ts" />
/// <reference path="models/Category.ts" />
/// <reference path="models/Product.ts" />
/// <reference path="models/CartItem.ts" />
/// <reference path="models/Promotion.ts" />
/// <reference path="models/Surcharge.ts" />
/// <reference path="models/OrderRequest.ts" />
/// <reference path="models/OrderResponse.ts" />
/// <reference path="interfaces/IPromotionScope.ts" />
/// <reference path="interfaces/ICategoryScope.ts" />
/// <reference path="interfaces/ICartScope.ts" />
/// <reference path="interfaces/IOrderScope.ts" />
/// <reference path="interfaces/ILoadingScope.ts" />
/// <reference path="interfaces/ICartRepository.ts" />
/// <reference path="interfaces/ICategoryResource.ts" />
/// <reference path="interfaces/ICategoryRepository.ts" />
/// <reference path="interfaces/IPromotionResource.ts" />
/// <reference path="interfaces/IPromotionRepository.ts" />
/// <reference path="interfaces/ISurchargeResource.ts" />
/// <reference path="interfaces/ISurchargeRepository.ts" />
/// <reference path="interfaces/IOrderRepository.ts" />
/// <reference path="interfaces/IOrderService.ts" />
/// <reference path="services/CategoryRepository.ts" />
/// <reference path="services/CartRepository.ts" />
/// <reference path="services/PromotionRepository.ts" />
/// <reference path="services/SurchargeRepository.ts" />
/// <reference path="services/OrderRepository.ts" />
/// <reference path="services/CategoryResourceFactory.ts" />
/// <reference path="services/PromotionResourceFactory.ts" />
/// <reference path="services/SurchargeResourceFactory.ts" />
/// <reference path="services/OrderService.ts" />
/// <reference path="controllers/PromotionController.ts" />
/// <reference path="controllers/CategoryController.ts" />
/// <reference path="controllers/CartController.ts" />
/// <reference path="controllers/OrderController.ts" />
/// <reference path="controllers/ApplicationController.ts" />
/// <reference path="controllers/ShopHomeController.ts" />
/// <reference path="controllers/ShopCategoryController.ts" />
/// <reference path="controllers/ShopOrderedController.ts" />